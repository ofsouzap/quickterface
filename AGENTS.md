...
**3. Stylesheet Generation (`stylesheet.ml` + `class.ml`)**
...
1. Add a new constructor to `Class.t` in `class.ml`:
   ```ocaml
   type t =
     | Main_container
     | My_new_class  (* Add your new class *)
     | ...
   ```
2. Define its style in `Stylesheet.class_style` function in `stylesheet.ml`:
   ```ocaml
...
**4. Page Body Construction (`app.ml`)**
...
**5. External Dependencies (`katex_setup.ml`)**
