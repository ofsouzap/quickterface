...
- **`web_app_backend/class.ml`**: CSS class name definitions
- **`web_app_backend/head.ml`**: Page head setup (title, meta tags, stylesheets)
- **`web_app_backend/utils.ml`**: Web utilities

##### Web Page Generation Architecture

The web application uses a minimal HTML file that delegates all page construction to OCaml code compiled to JavaScript via js_of_ocaml:

**1. Minimal index.html Structure**

Each web application uses a bare-bones HTML file that only loads the generated JavaScript:
```html
<!DOCTYPE html>
<html>
<body>
...
The `main.js` file is generated by js_of_ocaml from the OCaml web app implementation. All page construction happens in JavaScript/OCaml code.

**2. JavaScript Generation via js_of_ocaml**

The OCaml web app modules are compiled to JavaScript:
- Source: `*_web_app.ml` (implements `Web_app.Make(App)`)
- Compiled: `*.bc` (bytecode) → `*.bc.js` (JavaScript via js_of_ocaml)
- Build: `dune build path/to/*_web_app.bc.js`
- The generated JavaScript contains the entire application logic and page construction code

**3. Page Head Setup (`head.ml`)**

When the app starts, `Head.make` constructs the page `<head>` dynamically:
- **Title element**: Created via `create_title_element`, can be updated during app execution
- **Stylesheet element**: Created via `create_stylesheet_element`, contains CSS generated by `Stylesheet.css_string`
- **Viewport meta tag**: Sets mobile-friendly viewport settings (prevents zooming, etc.)
- **Charset meta tag**: Sets UTF-8 encoding
- **External dependencies**: KaTeX elements added asynchronously (see below)

All elements are appended to `document##.head` at runtime.

**4. Stylesheet Generation (`stylesheet.ml` + `class.ml`)**

CSS is generated programmatically in OCaml:

- **`Class.t`**: Enumeration of all CSS classes used in the application
  - Examples: `Main_container`, `Log_container`, `Log_item`, `Input_submit_button`, `Progress_bar_item`
  - Each class is prefixed with `quickterface__` to avoid naming conflicts
  - Generated using `[@@deriving to_string, enumerate]`

- **`Stylesheet.css_string`**: Complete CSS as a string
  - Defines styles for base elements (`body`, `button`, `input`, `select`)
  - Defines styles for all classes in `Class.t`
  - Includes responsive design (viewport units, flexbox layouts)
  - Includes mobile-friendly features (touch-action, safe-area-inset)
  - Custom pseudo-classes (e.g., `button:disabled`)
  - Generated at compile-time and injected into `<style>` tag at runtime

The CSS generation is modular:
```ocaml
(* Selector types *)
Selector_atom.t: Body | Button | Input | Select | Class of Class.t

(* Style entries *)
Style.Entry.t: { name: string; value: string }

(* CSS rules *)
Entry.t: { selector: Selector.t; style: Style.t }
```

**5. Page Body Construction (`app.ml`)**

The `App.make` function constructs the page body:
- Creates a main container div with the `Main_container` class
- Initializes the `Log.t` component (log display and input handling)
- Appends the main container to `document##.body`
- Returns a record `{ head; log }` representing the app state

**6. External Dependencies (`katex_setup.ml`)**

External libraries can be included dynamically:

- **KaTeX example**: Math rendering library
  - Creates `<link>` element for KaTeX CSS (from CDN with SRI integrity check)
  - Creates `<script>` element for KaTeX JavaScript (from CDN with SRI integrity check)
  - Uses `Utils.Await_load_element` to wait for resources to load before app starts
  - Elements are added to `document##.head` asynchronously via `Lwt`

This pattern can be extended for other external dependencies:
1. Create a setup module (e.g., `Library_setup`)
2. Define `make` function returning `Await_load_element.t` list
3. Add elements to head in `Head.make` with load awaiting
4. Use the library in output/input modules as needed

**Summary**: The web app backend uses a "code-first" approach where a minimal HTML file delegates all page construction to OCaml/JavaScript code. The backend handles head setup, stylesheet generation, body construction, and external dependency loading—providing a complete, self-contained web application from a single JavaScript file.
